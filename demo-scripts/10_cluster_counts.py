...
use more bins or fewer parameters. DALI uses emcee sampling but it is not slow
because of that. It is slower as it computes higher order derivatives.
Computation also depends on the derivative method backend from derivkit.
See derivkit documentation for more details.
"""
...
def build_prior_sigma_map(args, active_params):
    ...
    if args.prior_frac is not None:
        ...
        for p in active_params:
            ...
            if ref == 0.0:
                raise ValueError(
                    f"Cannot build relative prior for parameter '{p}' because "
                    f"its fiducial value is zero."
                )
