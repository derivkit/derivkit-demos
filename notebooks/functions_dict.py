...
test_functions = {
    "x_squared": {
        "func": lambda x: x**2,
        "label": r"$f(x) = x^2$",
        "reference": {1: lambda x: 2 * x, 2: lambda x: 2, 3: lambda x: 0},
    },
    "cubic_poly": {
        "func": lambda x: x**3 - 2 * x + 1,
        "label": r"$f(x) = x^3 - 2x + 1$",
        "reference": {1: lambda x: 3 * x**2 - 2, 2: lambda x: 6 * x, 3: lambda x: 6},
    },
    "sin_x": {
...
standard_functions = {
    "linear": {"func": lambda x: 2 * x + 3, "label": r"$f(x) = 2x + 3$"},
    "quadratic": {
        "func": lambda x: x**2 - 4 * x + 2,
        "label": r"$f(x) = x^2 - 4x + 2$",
    },
    "trigonometric": {"func": lambda x: np.sin(x), "label": r"$f(x) = \sin(x)$"},
    "cubic": {
        "func": lambda x: x**3 - 3 * x**2 + 2 * x + 1,
        "label": r"$f(x) = x^3 - 3x^2 + 2x + 1$",
    },
    "gaussian": {"func": lambda x: np.exp(-(x**2)), "label": r"$f(x) = e^{-x^2}$"},
}
...
blowup_functions = {
    "1_over_x": {"func": lambda x: 1 / x, "label": r"$f(x) = \frac{1}{x}$"},
    "log_x": {"func": lambda x: np.log(x), "label": r"$f(x) = \log(x)$"},
    "abs_x": {"func": lambda x: np.abs(x), "label": r"$f(x) = |x|$"},
    "step": {
        "func": lambda x: np.where(x < 0, -1, 1),
        "label": r"$f(x) = \mathrm{sign}(x)$",
    },
    "root_abs": {"func": lambda x: np.sqrt(np.abs(x)), "label": r"$f(x) = \sqrt{|x|}$"},
}
...
test_functions_residuals = [
    ...
    {"name": "x2_cos", "label": r"$x^2 \cos(x)$", "func": lambda x: x**2 * np.cos(x)},
    {
        "name": "x_exp_negx2",
        "label": r"$x\,e^{-x^2}$",
        "func": lambda x: x * np.exp(-(x**2)),
    },
    {
        "name": "gauss_sin_5x",
        "label": r"$e^{-x^2} \sin(5x)$",
        "func": lambda x: np.exp(-(x**2)) * np.sin(5 * x),
    },
    {
        "name": "logsin_combo",
        "label": r"$\ln(1 + x^2)\sin(x)$",
        "func": lambda x: np.log(1 + x**2) * np.sin(x),
    },
    {
        "name": "sin_cos",
        "label": r"$\sin(x)\cos(x)$",
    ...
    {
        "name": "piecewise_quad_sqrt",
        "label": r"$\begin{cases} x^2, & x<0 \\ \sqrt{x+1}, & x \geq 0 \end{cases}$",
        "func": lambda x: x**2 if x < 0 else np.sqrt(x + 1),
    },
    {
        "name": "piecewise_sin_cos",
        "label": r"$\begin{cases} \sin(x), & x<1 \\ \cos(x), & x \geq 1 \end{cases}$",
        "func": lambda x: np.sin(x) if x < 1 else np.cos(x),
    },
    ...
    {
        "name": "log_cos2_plus1",
        "label": r"$\ln(\cos^2(x) + 1)$",
        "func": lambda x: np.log(np.cos(x) ** 2 + 1),
    },
    {
        "name": "poly_exp_mix",
        "label": r"$x^3 \cos(x^2) + e^{-x^2}$",
        "func": lambda x: x**3 * np.cos(x**2) + np.exp(-(x**2)),
    },
    {
        "name": "log_abs_sin",
        "label": r"$\ln(|\sin(x)| + 1)$",
        "func": lambda x: np.log(np.abs(np.sin(x)) + 1),
    },
    {
        "name": "gauss_cos10x",
        "label": r"$e^{-x^2} \cos(10x)$",
        "func": lambda x: np.exp(-(x**2)) * np.cos(10 * x),
    },
    {
        "name": "atan_exp",
        "label": r"$\arctan(e^x)$",
        "func": lambda x: np.arctan(np.exp(x)),
    },
    {
        "name": "log_abs_sin_eps",
        "label": r"$\ln(|\sin(x)| + 10^{-3})$",
        "func": lambda x: np.log(np.abs(np.sin(x)) + 1e-3),
    },
    {
        "name": "sqrt_trig_identity",
        "label": r"$\sqrt{\sin^2(x) + \cos^2(x)}$",
        "func": lambda x: np.sqrt(np.sin(x) ** 2 + np.cos(x) ** 2),
    },
    {
        "name": "sigmoid",
    ...
    {
        "name": "gaussian_bump",
        "label": r"$\exp\left(-\frac{1}{1 - x^2}\right)$",
        "func": lambda x: np.exp(-1 / (1 - x**2)) if abs(x) < 1 else 0,
    },
    ...
    {"name": "sin_exp", "label": r"$\sin(e^x)$", "func": lambda x: np.sin(np.exp(x))},
    {
        "name": "cos_sin_x2",
        "label": r"$\cos(\sin(x^2))$",
        "func": lambda x: np.cos(np.sin(x**2)),
    },
    {
        "name": "sinc",
        "label": r"$\mathrm{sinc}(x)$",
