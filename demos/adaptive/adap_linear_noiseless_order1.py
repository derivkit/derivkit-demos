...
def main():
    ...
    # hollow samples: override facecolor/edgecolors only (style default is filled)
    ax.scatter(x_vis, y_vis, facecolor="none", edgecolors=blue, label="samples", zorder=3)

    # highlight x0 (also hollow)
    ax.scatter([x0], [f_clean(x0)], facecolor="none", edgecolors=red, label=r"$x_0$", zorder=4)

    # tangent line using default-settings derivative
    ax.plot(xx, f_clean(x0) + d_est * (xx - x0), color=blue, label=r"adaptive fit @ $x_0$")
    ...
    outfile = save_fig(fig, outdir, stem="adaptive_demo_linear_nonoise_order1", ext="png")
    plt.show()
